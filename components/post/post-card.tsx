...
export function PostCard({ author, content, timestamp, isThread }: PostCardProps) {
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-muted-foreground text-xs sm:text-sm truncate">@{author.username}</span>
            {timestamp && (
              <span className="text-muted-foreground text-xs sm:text-sm hidden sm:inline">· {timestamp}</span>
            )}
            <Button variant="ghost" size="icon" className="ml-auto h-6 w-6 flex-shrink-0">
